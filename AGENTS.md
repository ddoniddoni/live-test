...
모든 JavaScript 및 TypeScript 파일은 다음 import 규칙을 따른다.

1. import 문은 파일 최상단에 작성한다.
2. import는 다음 순서로 그룹을 나눈다.
   - React, Next.js 및 외부 라이브러리
   - `@/` 별칭을 사용하는 프로젝트 내부 파일
   - 현재 파일과 같은 폴더의 `./` 상대 경로 파일
3. 각 import 그룹 사이에는 빈 줄을 정확히 한 줄 둔다.
4. 외부 라이브러리 그룹에서는 React와 Next.js 관련 import를 먼저 작성한다.
5. `../`, `../../`, `../../../`처럼 상위 폴더로 이동하는 상대 경로 import는 사용하지 않는다.
6. 프로젝트 내부의 다른 폴더를 import할 때는 반드시 `@/` 별칭을 사용한다.
7. `./` 상대 경로는 현재 파일과 같은 폴더의 파일에만 사용한다.
8. 타입만 가져오는 경우에는 `import type`을 사용한다.
9. 중복 import를 작성하지 않는다.
10. 기존 파일을 수정할 때도 해당 파일의 import 구조를 이 규칙에 맞게 정리한다.

`next-env.d.ts`처럼 프레임워크가 생성·관리하는 파일은 직접 수정하지 않는다.
...
- CSS의 글자 크기, 여백, 간격, 고정 폭·높이, border-radius에는 정수 `px` 단위를 사용한다. `rem`과 소수점 `px`는 사용하지 않는다.
- 반응형 레이아웃 관계에는 `%`, `vw`, `vh`, `fr`, `minmax()`, `clamp()`를 사용할 수 있지만, `clamp()`의 최소·최대 고정값도 `px`로 작성한다.
